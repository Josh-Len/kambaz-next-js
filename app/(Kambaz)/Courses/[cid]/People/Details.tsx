...
export default function PeopleDetails({ uid, onClose }: PeopleDetailsProps) {
  ...
  const [name, setName] = useState("");      // full name in the input
  ...
  const fetchUser = async () => {
    ...
    setUser(fetched);
    // initialize the editable name with current first + last
    setName(`${fetched.firstName ?? ""} ${fetched.lastName ?? ""}`.trim());
    setEditing(false);
  };
  ...
  const saveUser = async () => {
    ...
    // naive split: first word = firstName, rest = lastName
  ...
  // nothing to show if no user selected
